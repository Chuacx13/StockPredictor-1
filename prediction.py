...
def data_munging_stocks(target_stock_name, target_stock_data, US_EU_market_dict, asia_market_dict):
    ...
    y = df.copy()[[target_stock_name]]
    scaled_y = scaler_y.fit_transform(y)

    scaler_x = MinMaxScaler()
    x = df.copy().drop(columns=[target_stock_name, 'Close', 'Price'])
    for col in x.columns:
        x[col] = scaler_x.fit_transform(x[[col]])

    df_scaled = pd.DataFrame(data=x, columns=x.columns)
...
def adjusted_metric(scaled_data, data, model, num_of_predictors, target_stock_name, scaler_y):  
    ...
    SST = ((data_copy[target_stock_name] - data_copy[target_stock_name].mean())**2).sum()
    SSR = ((data_copy['predicted_gain'] - data_copy[target_stock_name].mean())**2).sum()
    SSE = ((data_copy[target_stock_name] - data_copy['predicted_gain'])**2).sum()
    r2 = SSR/SST
...
def assess_table(train_scaled_data, test_scaled_data, train_data, test_data, _model, target_stock_name, _scaler_y):
    ...
    r2test, RMSEtest, predict_test_data = adjusted_metric(test_scaled_data, test_data, _model, num_of_predictors, target_stock_name, _scaler_y)
    r2train, RMSEtrain, predict_train_data = adjusted_metric(train_scaled_data, train_data, _model, num_of_predictors, target_stock_name, _scaler_y)
